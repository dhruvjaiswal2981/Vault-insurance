...
const HeaderLife = () => {
  ...
  return (
    <header className="w-full py-5 px-1 z-50 bg-[rgba(157,111,255,0.9)]">
      <div className="flex items-center w-full md:justify-around justify-between px-5">
        ...
        <div className="hidden md:flex items-center justify-center space-x-4">
          ...
          className="w-[170px] text-sm font-medium text-white px-6 py-2 border border-white rounded-lg transition hover:bg-purple-700 whitespace-nowrap cursor-pointer">
            Get Free Quote
          </button>
          <button
            onClick={handleClaimsClick}
            className="w-[170px] text-sm font-medium text-white border border-white px-6 py-2 rounded-lg transition hover:bg-purple-700 flex items-center justify-center gap-2 cursor-pointer"
          >
            Claims
            <svg
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#957CEB] shadow-lg rounded-lg mt-6 mx-4">
          <nav className="flex flex-col">
            {navLinks.map(({ name, path }) => (
              <a
                ...
                className={`px-4 py-4 text-lg text-white font-medium transition-colors ${
                  isActive(path)
                    ? "bg-[#9d8bda] bg-opacity-10 border-l-4 border-[#FFFF]"
                    : "hover:bg-[#9d8bda] hover:bg-opacity-10"
                }`}
            ...
            <div className="border-t border-gray-200 px-4 py-4 flex flex-col space-y-2">
              <button 
              onClick={handleGetQuote}
              className="w-full text-center text-sm font-medium text-white px-4 py-4 border border-white rounded-lg transition hover:bg-purple-700">
                Get Free Quote
              </button>
              <button
                onClick={handleClaimsClick}
                className="w-full text-center text-sm font-medium text-white border border-white px-4 py-4 rounded-lg transition hover:bg-purple-700"
              >
                Claims
              </button>
            </div>
